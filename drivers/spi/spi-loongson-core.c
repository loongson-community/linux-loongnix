...
{
	int cs;
	unsigned char mask = (BIT(4) | BIT(0)) << spi->chip_select;
	unsigned char val = en ? mask :  (BIT(0) << spi->chip_select);
				...
				struct spi_device *spi, struct spi_transfer *t)
{
	...
	if ((spi->mode ^ loongson_spi->mode) & (SPI_CPOL | SPI_CPHA))
...
int loongson_spi_init_controller(struct device *dev, void __iomem *regs)
{
	...
	controller = spi_alloc_master(dev, sizeof(struct loongson_spi));
	if (controller == NULL)
		return -ENOMEM;
	...
	if (!spi->clk_rate) {
		device_property_read_u64(dev, "clock-frequency", &spi->clk_rate);
		if (!spi->clk_rate) {
			spi->clk_rate = 100000000;
		}
	}
...
EXPORT_SYMBOL_GPL(loongson_spi_init_controller);
