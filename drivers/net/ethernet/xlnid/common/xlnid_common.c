...
STATIC s32 xlnid_get_san_mac_addr_offset(struct xlnid_hw *hw,
					 u16 *san_mac_offset);
...
s32 xlnid_init_ops_generic(struct xlnid_hw *hw)
{
	...
	u32 eec = XLNID_READ_REG_MAC(hw, SKYLAKE_EEC);
	...
	eeprom->ops.write_buffer = xlnid_write_eeprom_buffer_bit_bang_generic;
...
/**
 *  xlnid_read_pba_string_generic - Reads part number string from EEPROM
 *  @hw: pointer to hardware structure
				  ...
				  u32 pba_num_size)
{
	...
	DEBUGFUNC("xlnid_read_pba_string_generic");
	...
	ret_val = hw->eeprom.ops.read(hw, XLNID_PBANUM0_PTR, &data);
	if (ret_val) {
		DEBUGOUT("NVM Read Error\n");
		return ret_val;
	}

	ret_val = hw->eeprom.ops.read(hw, XLNID_PBANUM1_PTR, &pba_ptr);
	if (ret_val) {
		DEBUGOUT("NVM Read Error\n");
		return ret_val;
	}
	 ...
	 */
	if (data != XLNID_PBANUM_PTR_GUARD) {
		DEBUGOUT("NVM PBA number is not stored as string\n");

		/* we will need 11 characters to store the PBA */
		if (pba_num_size < 11) {
			DEBUGOUT("PBA string buffer too small\n");
			return XLNID_ERR_NO_SPACE;
		}
...
/**
 *  xlnid_write_eeprom_buffer_bit_bang_generic - Write EEPROM using bit-bang
 *  @hw: pointer to hardware structure
 ...
 **/
s32 xlnid_write_eeprom_buffer_bit_bang_generic(struct xlnid_hw *hw, u16 offset,
					       u16 words, u16 *data)
{
    return XLNID_SUCCESS;
}
 ...
 **/
s32 xlnid_read_eerd_generic(struct xlnid_hw *hw, u16 offset, u16 *data)
{
...
/**
 *  xlnid_get_san_mac_addr_offset - Get SAN MAC address offset from the EEPROM
 *  @hw: pointer to hardware structure
					 ...
					 u16 *san_mac_offset)
{
	...
	DEBUGFUNC("xlnid_get_san_mac_addr_offset");

	/*
	 * First read the EEPROM pointer to see if the MAC addresses are
	 * available.
	 */
	ret_val = hw->eeprom.ops.read(hw, XLNID_SAN_MAC_ADDR_PTR,
				      san_mac_offset);
	if (ret_val) {
		ERROR_REPORT2(XLNID_ERROR_INVALID_STATE,
			      "eeprom at offset %d failed",
			      XLNID_SAN_MAC_ADDR_PTR);
	}
...
s32 xlnid_get_san_mac_addr_generic(struct xlnid_hw *hw, u8 *san_mac_addr)
{
	...
	DEBUGFUNC("xlnid_get_san_mac_addr_generic");

	/*
	 * First read the EEPROM pointer to see if the MAC addresses are
	 * available.  If they're not, no point in calling set_lan_id() here.
	 */
	ret_val = xlnid_get_san_mac_addr_offset(hw, &san_mac_offset);
	...
	(hw->bus.func) ? (san_mac_offset += XLNID_SAN_MAC_ADDR_PORT1_OFFSET) :
			 (san_mac_offset += XLNID_SAN_MAC_ADDR_PORT0_OFFSET);
	for (i = 0; i < 3; i++) {
...
s32 xlnid_set_san_mac_addr_generic(struct xlnid_hw *hw, u8 *san_mac_addr)
{
	...
	ret_val = xlnid_get_san_mac_addr_offset(hw, &san_mac_offset);
	if (ret_val || san_mac_offset == 0 || san_mac_offset == 0xFFFF)
		return XLNID_ERR_NO_SAN_ADDR_PTR;
				 ...
				 u16 *wwpn_prefix)
{
	...
	*wwpn_prefix = 0xFFFF;
	...
	offset = XLNID_ALT_SAN_MAC_ADDR_BLK_PTR;
	...
	offset = alt_san_mac_blk_offset + XLNID_ALT_SAN_MAC_ADDR_CAPS_OFFSET;
	if (hw->eeprom.ops.read(hw, offset, &caps))
		goto wwn_prefix_err;
	if (!(caps & XLNID_ALT_SAN_MAC_ADDR_CAPS_ALTWWN))
		goto wwn_prefix_out;

	/* get the corresponding prefix for WWNN/WWPN */
	offset = alt_san_mac_blk_offset + XLNID_ALT_SAN_MAC_ADDR_WWNN_OFFSET;
	if (hw->eeprom.ops.read(hw, offset, wwnn_prefix)) {
		ERROR_REPORT2(XLNID_ERROR_INVALID_STATE,
			      "eeprom read at offset %d failed", offset);
	}

	offset = alt_san_mac_blk_offset + XLNID_ALT_SAN_MAC_ADDR_WWPN_OFFSET;
...
s32 xlnid_get_fcoe_boot_status_generic(struct xlnid_hw *hw, u16 *bs)
{
	...
	/* clear output first */
	*bs = xlnid_fcoe_bootstatus_unavailable;

	/* check if FCOE IBA block is present */
	offset = XLNID_FCOE_IBA_CAPS_BLK_PTR;
	status = hw->eeprom.ops.read(hw, offset, &caps);
	if (status != XLNID_SUCCESS)
		goto out;

	if (!(caps & XLNID_FCOE_IBA_CAPS_FCOE))
		goto out;

	/* check if iSCSI FCOE block is populated */
	status = hw->eeprom.ops.read(hw, XLNID_ISCSI_FCOE_BLK_PTR, &offset);
	if (status != XLNID_SUCCESS)
		goto out;
	...
	offset = offset + XLNID_ISCSI_FCOE_FLAGS_OFFSET;
	status = hw->eeprom.ops.read(hw, offset, &flags);
	if (status != XLNID_SUCCESS)
		goto out;

	if (flags & XLNID_ISCSI_FCOE_FLAGS_ENABLE)
		*bs = xlnid_fcoe_bootstatus_enabled;
	else
		*bs = xlnid_fcoe_bootstatus_disabled;
