...
{\
    .stat_string = _name, \
    .sizeof_stat = FIELD_SIZEOF(struct xlnid_adapter, _stat), \
                    ...
                    struct ethtool_link_ksettings *cmd)
{
    ...
    if (regval == XLNID_LINK_MODE_GEPHY)
    {
        supported |= SUPPORTED_LINK_MODE_GEPHY;
        cmd->base.port |= PORT_TP;
        adapter->hw.phy.type = xlnid_phy_unknown;
        adapter->hw.phy.media_type = xlnid_media_type_copper;
    }
    else if (regval == XLNID_LINK_MODE_1000BASEX)
    {
    ...
    if (supported_link & XLNID_LINK_SPEED_1GB_FULL)
    {
        ...
        supported |= SUPPORTED_1000baseX_Full;
    }
                ...
                struct ethtool_pauseparam *pause)
{
    ...
    {
        fc.requested_mode = xlnid_fc_rx_pause;
    }
    else if (pause->tx_pause)
    {
        fc.requested_mode = xlnid_fc_tx_pause;
    }
    else
    {
                       ...
                       u16 sw_idx)
{
    ...
    if (rule && (rule->sw_idx == sw_idx)) 
    {
        ...
        {
            err = xlnid_fdir_erase_perfect_filter_skylake(hw,
                                                         &rule->filter,
                                                          sw_idx);
            if (err)
            {
                    ...
                    struct ethtool_rxnfc *cmd)
{
    ...
    if (hlist_empty(&adapter->fdir_filter_list)) 
    {
        ...
        err = xlnid_fdir_set_input_mask_skylake(hw, &mask, adapter->cloud_mode);
    ...
    if (netif_running(adapter->netdev)) 
    {
        err = xlnid_fdir_write_perfect_filter_skylake(hw,
