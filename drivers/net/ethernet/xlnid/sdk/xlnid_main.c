...
static const struct pci_device_id xlnid_pci_tbl[] = {
	{PCI_VDEVICE(XEL, XLNID_DEV_ID_SKYLAKE), 0},
...
static u32 xlnid_read_reg_direct(struct xlnid_hw *hw, u32 reg, bool quiet)
{
	...
	if (unlikely(hw->phy.nw_mng_if_sel &
		     XLNID_NW_MNG_IF_SEL_SGMII_ENABLE)) {
		struct xlnid_adapter *adapter;
		int i;

		for (i = 0; i < 200; ++i) {
			value = readl(reg_addr + XLNID_MAC_SGMII_BUSY);
			if (likely(!value))
...
u32 xlnid_read_reg(struct xlnid_hw *hw, u32 reg, bool quiet, bool direct)
{
    ...
    if (!trys)
    {
        printk("xlnid_read_reg: trylock failed, reg=0x%08x\r\n", reg);
        regval = XLNID_FAILED_READ_REG;
    }
...
void xlnid_write_reg(struct xlnid_hw *hw, u32 reg, u32 value, bool direct)
{
    ...
    if (!trys)
    {
        printk("xlnid_write_reg: trylock failed, reg=0x%08x\r\n", reg);
    }
...
static inline void xlnid_irq_rearm_queues(struct xlnid_adapter *adapter,
					  u64 qmask)
{
	u32 mask;
			       ...
			       struct xlnid_ring *tx_ring, int napi_budget)
{
	...
	if (check_for_tx_hang(tx_ring) && xlnid_check_tx_hang(tx_ring)) {
		...
		e_err(drv, "Detected Tx Unit Hang%s\n"
			...
			tx_ring->queue_index,
			XLNID_READ_REG(hw, TDH(tx_ring->reg_idx)),
			XLNID_READ_REG(hw, TDT(tx_ring->reg_idx)),
 ...
 **/
static void xlnid_process_skb_fields(struct xlnid_ring *rx_ring,
				     union xlnid_adv_rx_desc *rx_desc,
				     struct sk_buff *skb)
{
...
static void xlnid_rx_skb(struct xlnid_q_vector *q_vector,
 ...
 **/
static bool xlnid_cleanup_headers(struct xlnid_ring __maybe_unused *rx_ring,
				  union xlnid_adv_rx_desc *rx_desc,
				  struct sk_buff *skb)
{
			      ...
			      unsigned int size)
{
	...
	unsigned int truesize = ring_uses_build_skb(rx_ring) ?
				SKB_DATA_ALIGN(XLNID_SKB_PAD + size) :
				SKB_DATA_ALIGN(size);
#endif
...
#ifdef HAVE_XDP_SUPPORT
static int xlnid_xmit_xdp_ring(struct xlnid_adapter *adapter,
#ifdef HAVE_XDP_FRAME_STRUCT
	      ...
	      struct xdp_buff __maybe_unused *xdp)
{
	...
	case XDP_TX:
		...
		result = xlnid_xmit_xdp_ring(adapter, xdpf);
#else
		result = xlnid_xmit_xdp_ring(adapter, xdp);
			       ...
			       int budget)
{
    ...
    {
		struct xlnid_ring *ring = adapter->xdp_ring[smp_processor_id()];
...
adjust_by_size:
	 ...
	 */
	if (itr & XLNID_ITR_ADAPTIVE_LATENCY)
		avg_wire_size >>= 1;
...
static void xlnid_set_itr(struct xlnid_q_vector *q_vector)
{
	...
	new_itr &= ~XLNID_ITR_ADAPTIVE_LATENCY;
	new_itr <<= 2;
...
int xlnid_poll(struct napi_struct *napi, int budget)
{
	...
	if (!test_bit(__XLNID_DOWN, &adapter->state))
		xlnid_irq_enable_queues(adapter, ((u64)1 << q_vector->v_idx));
...
#ifdef HAVE_DCBNL_IEEE
s32 xlnid_dcb_hw_ets(struct xlnid_hw *hw, struct ieee_ets *ets, int max_frame)
{
	...
	xlnid_dcb_calculate_tc_credits(ets->tc_tx_bw, refill, max, max_frame);
	return xlnid_dcb_hw_config(hw, refill, max,
...
static void xlnid_fdir_filter_restore(struct xlnid_adapter *adapter)
{
	...
	if (!hlist_empty(&adapter->fdir_filter_list))
		xlnid_fdir_set_input_mask_skylake(hw, &adapter->fdir_mask, 
				adapter->cloud_mode);

	hlist_for_each_entry_safe(filter, node2,
				  &adapter->fdir_filter_list, fdir_node) {
		xlnid_fdir_write_perfect_filter_skylake(hw,
				&filter->filter,
				filter->sw_idx,
...
static void xlnid_westlake_init(struct xlnid_adapter *adapter)
{
    ...
    //printk("XLNID_AUTO_MEDIA_SUPPORT link_up=%d\r\n", link_up);
...
static void xlnid_configure(struct xlnid_adapter *adapter)
{
	...
	if (adapter->flags & XLNID_FLAG_FDIR_HASH_CAPABLE) {
		xlnid_init_fdir_signature_skylake(&adapter->hw,
						adapter->fdir_pballoc);
	} else if (adapter->flags & XLNID_FLAG_FDIR_PERFECT_CAPABLE) {
		xlnid_init_fdir_perfect_skylake(&adapter->hw,
 ...
 **/
static int xlnid_close(struct net_device *netdev)
{
	struct xlnid_adapter *adapter = netdev_priv(netdev);
...
/* HAVE_TX_MQ */
    if (g_stats_fdir)
    {
        //hwstats->fdirmatch += XLNID_READ_REG(hw, FDIRMATCH);
	    //hwstats->fdirmiss += XLNID_READ_REG(hw, FDIRMISS);
    }
#endif /* HAVE_TX_MQ */
...
static void xlnid_fdir_reinit_subtask(struct xlnid_adapter *adapter)
{
	...
	if (xlnid_reinit_fdir_tables_skylake(hw) == XLNID_SUCCESS) {
		for (i = 0; i < adapter->num_tx_queues; i++)
...
static void xlnid_watchdog_update_link(struct xlnid_adapter *adapter)
{
    ...
    if (!(adapter->flags & XLNID_FLAG_NEED_LINK_UPDATE))
    {
        //printk("%s:%d: link_up=%d\r\n", __func__, __LINE__, adapter->link_up);
#ifdef XLNID_AUTO_MEDIA_SUPPORT
        if (adapter->link_up)
#endif
        {
    ...
    if (adapter->pdev->devfn == 0 && adapter->link_change)
    {
        ...
        if ((mode & XLNID_CRG_RX_PTSW_SOFTEN) == XLNID_CRG_RX_PTSW_SOFTEN)
        {
            ...
            if (loop <= 0)
            {
                ...
                printk("xlnid_watchdog_link_is_up: mode=0x%x\r\n", mode);
            }
        }
    }
        ...
        if (mode)
        {
            if ((mode & XLNID_CRG_RX_PTSW_SOFTEN) == XLNID_CRG_RX_PTSW_SOFTEN)
            {
                if (!(mode & XLNID_CRG_RX_PTSW_PORT))
                {
                    mode |= XLNID_CRG_RX_PTSW_PORT;
                    XLNID_WRITE_REG_MAC(&adapter->hw, WESTLAKE_CFG_PTSW_MODE, mode);
                    printk("xlnid_watchdog_link_is_down: mode=0x%x\r\n", mode);
                }
            }
            return;
        }
		     ...
		     u8 *hdr_len)
{
    ...
    /* 必须由硬件来执行校验和的计算才会往下走，否则直接返回 */
	if (skb->ip_summed != CHECKSUM_PARTIAL)
		return 0;

    /* 如果不是gso模式，则直接返回，gso模式即代表内核要求网卡进行tcp分段处理，
       否则代表内核已经进行过tcp分段无需网卡再分段 */
	if (!skb_is_gso(skb))
		return 0;
		      ...
		      struct xlnid_tx_buffer *first)
{
	...
	xlnid_fdir_add_signature_filter_skylake(&q_vector->adapter->hw,
				  ...
				  struct xlnid_ring *tx_ring)
{
	 ...
	 */
	 /* 获取所有分散存储的内存碎片数据，因为可能报文比较大，一块线性内存不够，那只能把碎片内存也用起来，这事内核的内存优化管理机制 */
 ...
 *    0    success
 */
static int xlnid_ptsw_ioctl(struct xlnid_adapter *adapter, struct ifreq *ifr)
{    
    struct xlnid_hw *hw = &adapter->hw;
    u32 regval = 0;
...
static int xlnid_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
    ...
    case SIOCENPTSW:
        return xlnid_ptsw_ioctl(adapter, ifr);
	default:
		return -EOPNOTSUPP;
	}
}
...
static void xlnid_xdp_ring_update_tail(struct xlnid_ring *ring)
{
    struct xlnid_adapter *adapter = ring->q_vector->adapter;
	/* Force memory writes to complete before letting h/w know there
	 * are new descriptors to fetch.
	 */
	wmb();

    if (adapter->hw.mac.type == xlnid_mac_SKYLAKE)
    {
	    writel(ring->next_to_use, ring->tail);
    }
    else
    {
        XLNID_WRITE_REG(&adapter->hw, TDT(ring->reg_idx), ring->next_to_use);
    }
}
...
#endif
{
	...
	for (i = 0; i < n; i++) {
		...
		err = xlnid_xmit_xdp_ring(adapter, xdpf);
		if (err != XLNID_XDP_TX) {
	...
	return n - drops;
#else
	err = xlnid_xmit_xdp_ring(adapter, xdp);
	if (err != XLNID_XDP_TX)
		return -ENOSPC;
...
/**
 * xlnid_set_fw_version - Set FW version
 ...
 *
 **/
static void xlnid_set_fw_version(struct xlnid_adapter *adapter)
{
	struct xlnid_hw *hw = &adapter->hw;
...
#endif
{
	...
	u8 part_str[XLNID_PBANUM_LENGTH];
	...
	err = xlnid_read_pba_string(hw, part_str, XLNID_PBANUM_LENGTH);
	if (err)
		strncpy(part_str, "Unknown", XLNID_PBANUM_LENGTH);
