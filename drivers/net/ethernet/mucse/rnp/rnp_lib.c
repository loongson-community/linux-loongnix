			      ...
			      int r_count, int step)
{
	...
	if (cpu_offset + v_idx - adapter->q_vector_off >
	    num_online_cpus()) {
...
void rnp_setup_layer2_remapping(struct rnp_hw *hw,
...
void rnp_setup_tuple5_remapping(struct rnp_hw *hw,
				union rnp_atr_input *input, u16 pri_id,
				u8 queue)
{
	u32 port = 0;
