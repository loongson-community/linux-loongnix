...
s32 rnpm_set_vfta_generic(struct rnpm_hw *hw, u32 vlan,
				u32 vind, bool vlan_on);
s32 rnpm_set_vfta_mac_generic(struct rnpm_hw *hw, u32 vlan,
				u32 vind, bool vlan_on);
...
void rnpm_set_rxpba_generic(struct rnpm_hw *hw, int num_pb,
...
static inline void hw_queue_strip_rx_vlan(struct rnpm_hw *hw,
	u8 ring_num, bool enable)
{
...
#define ptp_dbg(fmt, args...) \
	printk(KERN_DEBUG "ptp-dbg : " fmt, ##args)
#else
#define ptp_dbg(fmt, args...)
#endif
...
#define rnpm_info(fmt, args...) \
	printk(KERN_DEBUG "rnpm-info: " fmt, ##args)
#define rnpm_warn(fmt, args...) \
	printk(KERN_DEBUG "rnpm-warn: " fmt, ##args)
#define rnpm_err(fmt, args...) \
	printk(KERN_DEBUG "rnpm-err : " fmt, ##args)
...
static inline void  buf_dump_line(const char *msg,
	int line, void *buf, int len)
{
	int i, offset = 0;
	int msg_len = 1024;
	u8 msg_buf[1024];
...
#define rx_dbg(fmt, args...) \
	printk(KERN_DEBUG "KERN_DEBUG [ %s:%d ] " fmt, \
...
#define desc_hex_dump(msg, buf, len) \
	print_hex_dump(KERN_DEBUG, msg, DUMP_PREFIX_OFFSET, \
	16, 1, (buf), (len), false)
...
#define drection_dbg(fmt, arg...) \
	printk(KERN_DEBUG "[ %s:%d ] " fmt, __func__, \
	__LINE__, ##args)
#else
#define drection_dbg(fmt, arg...)
#endif
