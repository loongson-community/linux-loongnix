...
static const char rnpm_driver_string[] =
	"mucse 2/4/8port 10/40 Gigabit PCI Express Network Driver";
static char rnpm_default_device_descr[] __maybe_unused =
	"mucse(R) 2/4/8port 10/40 Gigabit Network Connection";
#define DRV_VERSION "0.1.5-rc49.loongson"
...
static void rnpm_htimer_start(struct rnpm_q_vector *q_vector)
{
	...
	hrtimer_start_range_ns(
		&q_vector->irq_miss_check_timer, ns_to_ktime(ns), ns, HRTIMER_MODE_REL);
}
...
static u32 rnpm_vlan_filter_status_update(struct rnpm_pf_adapter *pf_adapter)
{
	...
	spin_lock(&pf_adapter->vlan_filter_lock);
	pf_adapter->vlan_status_true = status;
	spin_unlock(&pf_adapter->vlan_filter_lock);
	return status;
}
 ...
 * @adapter: driver data
 */
static void rnpm_vlan_filter_disable(struct rnpm_adapter *adapter)
{
	struct rnpm_hw *hw = &adapter->hw;
	struct rnpm_pf_adapter *pf_adapter = adapter->pf_adapter;
	u8 port = adapter->port;
 ...
 * @adapter: driver data
 */
static void rnpm_vlan_filter_enable(struct rnpm_adapter *adapter)
{
	struct rnpm_hw *hw = &adapter->hw;
	struct rnpm_pf_adapter *pf_adapter = adapter->pf_adapter;
	u8 port = adapter->port;
...
static int rnpm_sw_init(struct rnpm_adapter *adapter)
{
	...
	adapter->tx_work_limit = RNPM_DEFAULT_TX_WORK;
	adapter->rx_usecs = RNPM_PKT_TIMEOUT;
	adapter->rx_frames = RNPM_RX_PKT_POLL_BUDGET;
	adapter->tx_usecs = RNPM_PKT_TIMEOUT_TX;
	adapter->tx_frames = RNPM_TX_PKT_POLL_BUDGET;
	...
	adapter->tx_ring_item_count = RNPM_DEFAULT_TXD;
	adapter->rx_ring_item_count = RNPM_DEFAULT_RXD;
...
void rnpm_update_stats(struct rnpm_adapter *adapter)
{
	...
	net_stats->rx_crc_errors = rd32(hw, RNPM_RXTRANS_CRC_ERR_PKTS(port));

	// net_stats->rx_crc_errors = rd32(hw, RNPM_RXTRANS_CRC_ERR_PKTS(port));

	net_stats->rx_errors = rd32(hw, RNPM_RXTRANS_WDT_ERR_PKTS(port)) +
 ...
 * @adapter: pointer to the device adapter structure
 **/
static void rnpm_watchdog_flush_tx(struct rnpm_adapter *adapter)
{
	int i;
...
#define TIMEOUT_COUNT (1000)
	...
	if (times >= TIMEOUT_COUNT) {
		rnpm_dbg("wait axi ready timeout\n");
		return -1;
	}
...
void wait_all_port_resetting(struct rnpm_pf_adapter *pf_adapter)
{
	int i;
	struct rnpm_adapter *adapter;
	// should wait all
...
void clean_all_port_resetting(struct rnpm_pf_adapter *pf_adapter)
{
	int i;
	struct rnpm_adapter *adapter;
	// should wait all
...
static void rnpm_pf_reset_subtask(struct rnpm_pf_adapter *pf_adapter)
{
	...
	wait_all_port_resetting(pf_adapter);
	printk("rx hang detected, reset pf \n");
	// first stop all port
	for (i = pf_adapter->adapter_cnt - 1; i >= 0; i--) {
		adapter = pf_adapter->adapter[i];
		if (!adapter) {
	...
	clean_all_port_resetting(pf_adapter);
}
...
err_msix:
	pcim_iounmap(pdev, pf_adapter->hw_bar2);
	// err_ioremap2:
		...
		struct rnpm_pf_adapter **ppf_adapter)
{
	...
	if (ppf_adapter)
		devm_kfree(&pdev->dev, *ppf_adapter);
