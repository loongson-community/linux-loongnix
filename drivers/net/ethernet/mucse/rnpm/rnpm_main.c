...
#define ASSIN_PDEV
#endif
static const char rnpm_driver_string[] = "mucse 4/8port 1/10 Gigabit PCI Express Network Driver";
...
static const char rnpm_copyright[] = "Copyright (c) 2020-2023 mucse Corporation.";
...
static bool rnpm_alloc_mapped_page(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *bi,
								   union rnpm_rx_desc *rx_desc, u16 bufsz, u64 fun_id);
static void rnpm_put_rx_buffer(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *rx_buffer);
...
static bool rnpm_alloc_mapped_skb(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *bi);
#endif
...
static struct pci_device_id rnpm_pci_tbl[] = {
	...
	*/
	...
	{PCI_DEVICE(PCI_VENDOR_ID_MUCSE, 0x1021), .driver_data = board_n400_4x1G},
...
MODULE_PARM_DESC(force_speed_ablity_pf0, "allow to force speed 1/10G for fiber on pf0");

static int force_speed_ablity_pf1;
module_param(force_speed_ablity_pf1, uint, 0000);
MODULE_PARM_DESC(force_speed_ablity_pf1, "allow to force speed 1/10G for fiber on pf1");

MODULE_PARM_DESC(fw_10g_1g_auto_det,
...
// #define RNPM_DEBUG_KYLIN_SM
#ifdef RNPM_DEBUG_KYLIN_SM
static void buf_dump_skb(void *buf, int len)
{
	int i, offset = 0;
	int msg_len = 1024;
	u8	msg_buf[1024];
	...
	offset += snprintf(msg_buf + offset, msg_len, "=== data:%p==\n000: ", buf);
...
static u16 udp_checksum(void *buf, struct iphdr *iph)
{
	...
	// printk("saddr %x daddr %x protocol %x len %x\n", ph4.saddr, ph4.daddr, ph4.protocol,
	// ph4.payload_len);
...
__maybe_unused void check_udp_chksum(void *data, struct rnpm_rx_buffer *rx_buffer)
{
	...
	if (iph->protocol == IPPROTO_UDP && ntohs(iph->tot_len) == 1500) {
		...
		if (csum != csum_temp) {
			printk("chksum error origin %x, cal %x %llx\n", csum_temp, csum, rx_buffer->dma);
			buf_dump_skb(data, 1514);
		}
 ...
 *
 */
static void rnpm_set_ring_vector(struct rnpm_adapter *adapter, u8 rnpm_queue, u8 rnpm_msix_vector)
{
	...
	DPRINTK(IFUP, INFO, "Set Ring-Vector queue:%d (reg:0x%x) <-- Rx-MSIX:%d, Tx-MSIX:%d\n",
			rnpm_queue, RING_VECTOR(rnpm_queue), rnpm_msix_vector, rnpm_msix_vector);

	rnpm_wr_reg(hw->ring_msix_base + RING_VECTOR(rnpm_queue), data);
}

static inline void rnpm_irq_rearm_queues(struct rnpm_adapter *adapter, u64 qmask)
{
	// u32 mask;
}

void rnpm_unmap_and_free_tx_resource(struct rnpm_ring *ring, struct rnpm_tx_buffer *tx_buffer)
{
	if (tx_buffer->skb) {
		...
		dma_unmap_page(ring->dev, dma_unmap_addr(tx_buffer, dma), dma_unmap_len(tx_buffer, len),
					   DMA_TO_DEVICE);
	}
...
static void rnpm_check_restart_tx(struct rnpm_q_vector *q_vector, struct rnpm_ring *tx_ring)
{
 ...
 **/
static bool rnpm_clean_tx_irq(struct rnpm_q_vector *q_vector, struct rnpm_ring *tx_ring,
							  int napi_budget)
{
	struct rnpm_adapter	  *adapter = q_vector->adapter;
	...
	do {
		...
		while (tx_desc != eop_desc) {
			/* print desc */
			buf_dump_line("desc %d  ", i + tx_ring->count, tx_desc, sizeof(*tx_desc));
...
static inline void rnpm_rx_hash(struct rnpm_ring *ring, union rnpm_rx_desc *rx_desc,
								struct sk_buff *skb)
{
	int rss_type;
 ...
 **/
static inline void rnpm_rx_checksum(struct rnpm_ring *ring, union rnpm_rx_desc *rx_desc,
									struct sk_buff *skb)
{
	...
	/* vxlan packet handle ? */
	if (rnpm_get_stat(rx_desc, RNPM_RXD_STAT_TUNNEL_MASK) == RNPM_RXD_STAT_TUNNEL_VXLAN) {
		encap_pkt = true;
...
void rnpm_alloc_rx_buffers(struct rnpm_ring *rx_ring, u16 cleaned_count)
{
	...
	do {
		...
		while (count < rx_ring->rx_page_buf_nums && cleaned_count) {
			...
			bi->page_offset = rx_ring->rx_per_buf_mem * count + rnpm_rx_offset(rx_ring);
			/* map page for use */
			dma = dma_map_page_attrs(rx_ring->dev, page, bi->page_offset, bufsz, DMA_FROM_DEVICE,
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
									 &attrs);
#else
			...
			dma_sync_single_range_for_device(rx_ring->dev, bi->dma, 0, bufsz, DMA_FROM_DEVICE);
...
void rnpm_alloc_rx_buffers(struct rnpm_ring *rx_ring, u16 cleaned_count)
{
	...
	do {
		...
		if (!rnpm_alloc_mapped_page(rx_ring, bi))
			break;
		dma_sync_single_range_for_device(rx_ring->dev, bi->dma, bi->page_offset, bufsz,
										 DMA_FROM_DEVICE);
#endif
...
static void rnpm_set_rsc_gso_size(struct rnpm_ring *ring, struct sk_buff *skb)
{
	...
	skb_shinfo(skb)->gso_size = DIV_ROUND_UP((skb->len - hdr_len), RNPM_CB(skb)->append_cnt);
	skb_shinfo(skb)->gso_type = SKB_GSO_TCPV4;
}

__maybe_unused static void rnpm_update_rsc_stats(struct rnpm_ring *rx_ring, struct sk_buff *skb)
{
						 ...
						 struct sk_buff *skb)
{
	...
	if ((netdev_ring(rx_ring)->features & NETIF_F_HW_VLAN_RX) &&
		...
		__vlan_hwaccel_put_tag(skb, htons(ETH_P_8021Q), le16_to_cpu(rx_desc->wb.vlan));
	}
#else
 ...
 **/
static void rnpm_process_skb_fields(struct rnpm_ring *rx_ring, union rnpm_rx_desc *rx_desc,
									struct sk_buff *skb)
{
...
static void rnpm_receive_skb(struct rnpm_q_vector *q_vector, struct sk_buff *skb)
{
 ...
 **/
static inline void rnpm_add_active_tail(struct sk_buff *head, struct sk_buff *tail)
{
 ...
 **/
static bool rnpm_is_non_eop(struct rnpm_ring *rx_ring, union rnpm_rx_desc *rx_desc)
{
...
static bool rnpm_alloc_mapped_page(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *bi)
{
	...
	page = alloc_pages(GFP_ATOMIC | __GFP_COLD | __GFP_COMP, rnpm_rx_pg_order(rx_ring));
	if (unlikely(!page)) {
		rx_ring->rx_stats.alloc_rx_page_failed++;
		return false;
	}
	...
	/* map page for use */
	dma = dma_map_page_attrs(rx_ring->dev, page, 0, rnpm_rx_pg_size(rx_ring), DMA_FROM_DEVICE,
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
							 &attrs);
#else
							 RNPM_RX_DMA_ATTR);
 ...
 * chain.
 */
__maybe_unused static void rnpm_dma_sync_frag(struct rnpm_ring *rx_ring, struct sk_buff *skb)
{
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
	DEFINE_DMA_ATTRS(attrs);
	...
	if (unlikely(RNPM_CB(skb)->page_released)) {
		dma_unmap_page_attrs(rx_ring->dev, RNPM_CB(skb)->dma, rnpm_rx_pg_size(rx_ring),
							 DMA_FROM_DEVICE,
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
							 &attrs);
#else
		...
		dma_sync_single_range_for_cpu(rx_ring->dev, RNPM_CB(skb)->dma, offset, skb_headlen(skb),
									  DMA_FROM_DEVICE);
	} else {
		skb_frag_t *frag = &skb_shinfo(skb)->frags[0];

		dma_sync_single_range_for_cpu(rx_ring->dev, RNPM_CB(skb)->dma, skb_frag_off(frag),
									  skb_frag_size(frag), DMA_FROM_DEVICE);
	}
}

/* drop this packets if error */
static bool rnpm_check_csum_error(struct rnpm_ring *rx_ring, union rnpm_rx_desc *rx_desc,
								  unsigned int size, unsigned int *driver_drop_packets)
{
	bool err = false;
	...
	if (netdev->features & NETIF_F_RXCSUM) {
		if (unlikely(rnpm_test_staterr(rx_desc, RNPM_RXD_STAT_ERR_MASK))) {
			rx_debug_printk(
				...
				(rx_ring->q_vector->adapter->flags & RNPM_FLAG_SRIOV_ENABLED) ? "On" : "Off",
				rx_desc->wb.mark, rx_desc->wb.cmd);
			/* push this packet to stack if in promisc mode */
			rx_ring->rx_stats.csum_err++;

			if ((!(netdev->flags & IFF_PROMISC) && (!(netdev->features & NETIF_F_RXALL)))) {
				// if not ipv4 with l4 error, we should ignore l4 csum error
				if (unlikely(rnpm_test_staterr(rx_desc, RNPM_RXD_STAT_L4_MASK) &&
							 (!(rx_desc->wb.rev1 & RNPM_RX_L3_TYPE_MASK)))) {
					rx_ring->rx_stats.csum_err--;
					goto skip_fix;
				}

				if (unlikely(rnpm_test_staterr(rx_desc, RNPM_RXD_STAT_SCTP_MASK))) {
					if ((size > 60) && (rx_desc->wb.rev1 & RNPM_RX_L3_TYPE_MASK)) {
						err = true;
					} else {
...
skip_fix:
	if (err) {
...
#else
		unsigned int truesize = ring_uses_build_skb(rx_ring) ? SKB_DATA_ALIGN(RNPM_SKB_PAD + size)
															 : SKB_DATA_ALIGN(size);
#endif
		...
		dma_sync_single_range_for_cpu(rx_ring->dev, rx_buffer->dma, rx_buffer->page_offset,
									  RNPM_RXBUFFER_1536, DMA_FROM_DEVICE);
 ...
 **/
static void rnpm_reuse_rx_page(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *old_buff)
{
 ...
 **/
static void rnpm_add_rx_frag(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *rx_buffer,
							 struct sk_buff *skb, unsigned int size)
{
	...
	unsigned int truesize =
		ring_uses_build_skb(rx_ring) ? SKB_DATA_ALIGN(RNPM_SKB_PAD + size) : SKB_DATA_ALIGN(size);
#endif

	skb_add_rx_frag(skb, skb_shinfo(skb)->nr_frags, rx_buffer->page, rx_buffer->page_offset, size,
					truesize);
												 ...
												 const unsigned int	 size)
{
	...
	rx_buf_dump("rx buf", page_address(rx_buffer->page) + rx_buffer->page_offset, rx_desc->wb.len);

	/* we are reusing so sync this buffer for CPU use */
	dma_sync_single_range_for_cpu(rx_ring->dev, rx_buffer->dma, 0, size, DMA_FROM_DEVICE);
	/* skip_sync: */
	rx_buffer->pagecnt_bias--;
...
static void rnpm_put_rx_buffer(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *rx_buffer)
{
	...
	if (rnpm_can_reuse_rx_page(rx_buffer)) {
		...
		dma_unmap_page_attrs(rx_ring->dev, rx_buffer->dma, rnpm_rx_bufsz(rx_ring), DMA_FROM_DEVICE,
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
							 &attrs);
#else
							 RNPM_RX_DMA_ATTR);
							   ...
							   struct sk_buff *skb)
{
	...
	if (rnpm_can_reuse_rx_page(rx_buffer)) {
		...
		//	} else {
		/* we are not reusing the buffer so unmap it */
		dma_unmap_page_attrs(rx_ring->dev, rx_buffer->dma, rnpm_rx_pg_size(rx_ring),
							 DMA_FROM_DEVICE,
#if defined(HAVE_STRUCT_DMA_ATTRS) && defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
							 &attrs);
#else
...
static struct sk_buff *rnpm_construct_skb(struct rnpm_ring		*rx_ring,
										  struct rnpm_rx_buffer *rx_buffer,
										  union rnpm_rx_desc *rx_desc, unsigned int size)
{
	...
	// headlen = eth_get_headlen(skb->dev, va, RNPM_RX_HDR_SIZE);
	...
	if (size) {
		...
		skb_add_rx_frag(skb, 0, rx_buffer->page, (va + headlen) - page_address(rx_buffer->page),
						size, truesize);
		rx_buffer->page_offset += truesize;
	} else {
...
static struct sk_buff *rnpm_build_skb(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *rx_buffer,
									  union rnpm_rx_desc *rx_desc, unsigned int size)
{
	void		*va = page_address(rx_buffer->page) + rx_buffer->page_offset;
	unsigned int truesize =
		SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) + SKB_DATA_ALIGN(size + RNPM_SKB_PAD);
	struct sk_buff *skb;
...
#else
static struct sk_buff *rnpm_construct_skb(struct rnpm_ring		*rx_ring,
										  struct rnpm_rx_buffer *rx_buffer, struct xdp_buff *xdp,
										  union rnpm_rx_desc *rx_desc)
{
	...
	unsigned int truesize = rnpm_rx_pg_size(rx_ring) / 2;
#else
	unsigned int truesize = SKB_DATA_ALIGN(xdp->data_end - xdp->data_hard_start);
#endif
	struct sk_buff *skb;
	...
	if (size > RNPM_RX_HDR_SIZE) {
		...
		*/
		...
		skb_add_rx_frag(skb, 0, rx_buffer->page, xdp->data - page_address(rx_buffer->page), size,
						truesize);
#if (PAGE_SIZE < 8192)
		rx_buffer->page_offset ^= truesize;
#else
...
#ifdef HAVE_SWIOTLB_SKIP_CPU_SYNC
static struct sk_buff *rnpm_build_skb(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *rx_buffer,
									  struct xdp_buff *xdp, union rnpm_rx_desc *rx_desc)
{
...
static void rnpm_rx_buffer_flip(struct rnpm_ring *rx_ring, struct rnpm_rx_buffer *rx_buffer,
								unsigned int size)
{
#if (PAGE_SIZE < 8192)
	unsigned int truesize = rnpm_rx_pg_size(rx_ring) / 2;
...
#ifdef RNPM_OPTM_WITH_LPAGE
static int rnpm_clean_rx_irq(struct rnpm_q_vector *q_vector, struct rnpm_ring *rx_ring, int budget)
{
	...
	*/
	...
	while (likely(total_rx_packets < budget)) {
		...
		if (rnpm_check_csum_error(rx_ring, rx_desc, size, &driver_drop_packets)) {
			cleaned_count++;
			continue;
		}
		...
		if (skb) {
			...
			skb = rnpm_construct_skb(rx_ring, rx_buffer, rx_desc, size);
		}
...
#ifndef NO_PTP
		if (module_enable_ptp && adapter->ptp_rx_en && adapter->flags2 & RNPM_FLAG2_PTP_ENABLED) {
	...
	while (likely(total_rx_packets < budget)) {
		...
		*/
		...
		if (!RNPM_CB(skb)->head) {
			...
			dma_unmap_single(rx_ring->dev, rx_buffer->dma, rx_ring->rx_buf_len, DMA_FROM_DEVICE);
		}
		...
		/* unmap first */
		dma_unmap_single(rx_ring->dev, RNPM_CB(skb)->dma, rx_ring->rx_buf_len, DMA_FROM_DEVICE);
...
static void rnpm_configure_msix(struct rnpm_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_q_vectors; i++) {
		...
		rnpm_for_each_ring(ring, q_vector->rx)
			rnpm_set_ring_vector(adapter, ring->rnpm_queue_idx, q_vector->v_idx);
	}
}
...
void rnpm_write_eitr(struct rnpm_q_vector *q_vector, bool is_rxframe)
{
	...
	if (is_rxframe) {
		rnpm_for_each_ring(ring, q_vector->rx)
		{
			wr32(hw, RNPM_DMA_REG_RX_INT_DELAY_PKTCNT(ring->rnpm_queue_idx), q_vector->itr);
		}
	} else {
		rnpm_for_each_ring(ring, q_vector->rx)
		{
			wr32(hw, RNPM_DMA_REG_RX_INT_DELAY_TIMER(ring->rnpm_queue_idx), itr_reg);
		}
	}
}
							...
							struct rnpm_ring_container *ring_container)
{
	...
	if (time_after_eq(next_update, ring_container->next_update)) {
		...
		if (rnpm_container_is_rx(q_vector, ring_container) && (speed > SPEED_100)) {
			 ...
			 */
			if (packets && packets < (24 + off_1) && bytes < 12112 * DIV_ROUND_UP(factor, 2)) {
				if ((packets <= 3) && (avg_wire_size <= 1120) && (avg_wire_size >= 768))
					itr = 2;
				else
					itr = 1;
				goto clear_counts;
		...
		if (ring_container->poll_times && factor) {
			t = (ring_container->poll_times > factor) ? ring_container->poll_times / factor : 1;
		...
		if (poll_packets <= (32 + off_2)) {
			if ((poll_packets <= 3) && (avg_wire_size <= 1120) && (speed > SPEED_100)) {
				/* 1K - 2K bytes*/
				itr = 2;
			} else {
				if (speed >= SPEED_10000) {
					/* 10G */
					if (((poll_packets - itr) == 1) /*|| ((poll_packets - itr) == 2)*/) {
						/* Hold this itr */
					} else {
						if (poll_packets == itr) {
					...
					/* 100M/10M */
					// if (((poll_packets - itr) == 1) /*|| ((poll_packets -
					// itr) == 2)*/) {
					if (((poll_packets - itr) == 1) &&
						(itr != 1) /*|| ((poll_packets - itr) == 2)*/) {
						/* Hold this itr */
					} else {
						if (poll_packets >= itr) {
							t = DIV_ROUND_UP(poll_packets - itr, 2);
...
clear_counts:
	// printk("packets=%d bytes=%d poll_packets=%d itr=%d q=%d times=%d
	// speed=%d\n", 	packets, bytes, poll_packets, itr, q_vector->itr,
	// ring_container->poll_times, speed);
	/* write back value */
 ...
 **/
static bool __maybe_unused rnpm_update_itr(struct rnpm_q_vector		  *q_vector,
										   struct rnpm_ring_container *ring_container)
{
	// unsigned int itr = RNPM_ITR_ADAPTIVE_MIN_USECS |
	// RNPM_ITR_ADAPTIVE_LATENCY;
	...
	if (rnpm_container_is_rx(q_vector, ring_container)) {
		...
		if (packets && packets < 24 && bytes < 12112) {
			...
			avg_wire_size = clamp_t(unsigned int, avg_wire_size, 128, 12800);
			// printk("sp packets=%d bytes=%d itr=%d avg_wire_size=%d\n",
			// packets, bytes, itr, avg_wire_size);
	...
	if (packets < 48) {
		...
		if (packets && packets < 8) {
			...
			// printk("<8 packets=%d itr=%d\n", packets, itr);
		} else {
			// ring_container->itrf = 16;
...
adjust_for_speed:
	...
	switch (q_vector->adapter->link_speed) {
		...
		case RNPM_LINK_SPEED_100_FULL:
		default:
			itr += DIV_ROUND_UP(avg_wire_size, RNPM_ITR_ADAPTIVE_MIN_INC * 256) *
...
clear_counts:
	...
	if (ring_container->itr >= (itr + 12)) {
		ring_container->itr = (ring_container->itr >> 1) + RNPM_ITR_ADAPTIVE_MIN_INC;
	} else {
		ring_container->itr = itr;
	}
...
__maybe_unused static void rnpm_check_sfp_event(struct rnpm_adapter *adapter, u32 eicr)
{
	// struct rnpm_hw *hw = &adapter->hw;
}
...
static inline void rnpm_irq_disable_queues(struct rnpm_q_vector *q_vector)
{
	...
	rnpm_for_each_ring(ring, q_vector->tx)
	{
		...
		// q_vector->new_rx_count); 	q_vector->old_rx_count =
...
static void rnpm_htimer_start(struct rnpm_q_vector *q_vector)
{
	...
	hrtimer_start_range_ns(&q_vector->irq_miss_check_timer, ns_to_ktime(ns), ns,
...
{
	struct rnpm_q_vector *q_vector = container_of(napi, struct rnpm_q_vector, napi);
	struct rnpm_adapter	 *adapter  = q_vector->adapter;
	struct rnpm_hw		 *hw	   = &adapter->hw;
	struct rnpm_ring	 *ring;
	...
	rnpm_for_each_ring(ring, q_vector->rx)
	{
		...
		if (unlikely(ring->ring_flags & RNPM_RING_FLAG_DELAY_SETUP_RX_LEN)) {
			...
			head = rd32(hw, RNPM_DMA_REG_RX_DESC_BUF_HEAD(ring->rnpm_queue_idx));
			if (head < RNPM_MIN_RXD) {
				...
				// start rx again
				wr32(hw, RNPM_DMA_RX_START(ring->rnpm_queue_idx), 1);
			}
		}
		work_done += cleaned;
	...
	if (!clean_complete) {
		...
		if (!cpumask_test_cpu(cpu_id, &q_vector->affinity_mask)) {
			...
			smp_mb();
			rnpm_for_each_ring(ring, q_vector->tx) rnpm_check_restart_tx(q_vector, ring);
#endif
 ...
 **/
static void rnpm_irq_affinity_notify(struct irq_affinity_notify *notify, const cpumask_t *mask)
{
	struct rnpm_q_vector *q_vector = container_of(notify, struct rnpm_q_vector, affinity_notify);
...
static int rnpm_request_msix_irqs(struct rnpm_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_q_vectors; i++) {
		...
		if (q_vector->tx.ring && q_vector->rx.ring) {
			snprintf(q_vector->name, sizeof(q_vector->name) - 1, "%s-%s-%d-%d", netdev->name,
					 "TxRx", i, q_vector->v_idx);
		} else {
			WARN(!(q_vector->tx.ring && q_vector->rx.ring), "%s vector%d tx rx is null, v_idx:%d\n",
				 netdev->name, i, q_vector->v_idx);
			/* skip this unused q_vector */
			continue;
		}
		err = request_irq(entry->vector, &rnpm_msix_clean_rings, 0, q_vector->name, q_vector);
		if (err) {
			e_err(probe,
...
int rx_poll_thread_handler(void *data)
{
	...
	do {
		for (i = 0; i < adapter->num_q_vectors; i++) rnpm_msix_clean_rings(0, adapter->q_vector[i]);
...
#ifdef DISABLE_RX_IRQ
	adapter->rx_poll_thread = kthread_run(rx_poll_thread_handler, adapter, adapter->name);
...
int rnpm_xmit_nop_frame_ring(struct rnpm_adapter *adapter, struct rnpm_ring *tx_ring)
{
...
int rnpm_xmit_nop_frame_ring_temp(struct rnpm_adapter *adapter, struct rnpm_ring *tx_ring)
{
	u16					 i = tx_ring->next_to_use;
	struct rnpm_tx_desc *tx_desc;
	tx_desc = RNPM_TX_DESC(tx_ring, i);
 ...
 **/
void rnpm_configure_tx_ring(struct rnpm_adapter *adapter, struct rnpm_ring *ring)
{
	struct rnpm_hw *hw = &adapter->hw;
	// int i;
	...
	*/
	...
	wr32(hw, RNPM_DMA_REG_TX_INT_DELAY_TIMER(queue_idx), adapter->tx_usecs * hw->usecstocount);
...
__maybe_unused static void rnpm_rx_desc_queue_enable(struct rnpm_adapter *adapter,
													 struct rnpm_ring	 *ring)
{
	// struct rnpm_hw *hw = &adapter->hw;
...
void rnpm_configure_rx_ring(struct rnpm_adapter *adapter, struct rnpm_ring *ring)
{
	struct rnpm_hw *hw		 = &adapter->hw;
	...
	wr32(hw, RNPM_DMA_REG_RX_INT_DELAY_TIMER(q_idx), adapter->rx_usecs * hw->usecstocount);
...
static void rnpm_configure_virtualization(struct rnpm_adapter *adapter)
{
	...
	wr32(hw, RNPM_ETH_DMAC_FCTRL, rd32(hw, RNPM_ETH_DMAC_FCTRL) | RNPM_FCTRL_BROADCASE_BYPASS);
	...
	// mac[5]; 			machi = (mac[0] << 8) | mac[1]; 			wr32(hw,
	// RNPM_DMA_PORT_VBE_MAC_LO_TBL(port, vfnum), maclow); 			wr32(hw,
	// RNPM_DMA_PORT_VBE_MAC_HI_TBL(port, vfnum), machi); 			ring |=
	// ((0x80 | vfnum)
	//<< 8); 			wr32(hw, RNPM_DMA_PORT_VEB_VF_RING_TBL(port, vfnum),
	// ring);
	//		}
	//	} else {
	//		vfnum = RNPM_MAX_VF_CNT - 1;
...
static void rnpm_set_rx_buffer_len(struct rnpm_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
...
#if (PAGE_SIZE < 8192)
		if (RNPM_2K_TOO_SMALL_WITH_PADDING || (max_frame > (ETH_FRAME_LEN + ETH_FCS_LEN)))
			;
		...
		rx_ring->rx_per_buf_mem =
			ALIGN((rnpm_rx_offset(rx_ring) + rnpm_rx_bufsz(rx_ring) +
				   SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) + RNPM_RX_HWTS_OFFSET),
				  1024);
#endif
	}
}
...
static void rnpm_configure_rx(struct rnpm_adapter *adapter)
{
	...
	if (adapter->pf_adapter->default_rx_ring > 0) {
		wr32(hw, RNPM_ETH_DEFAULT_RX_RING, adapter->pf_adapter->default_rx_ring);
	}

#if (PAGE_SIZE < 8192)
	hw->dma_split_size =
		rnpm_rx_pg_size(rx_ring) / 2 - rnpm_rx_offset(rx_ring) - sizeof(struct skb_shared_info);
#endif
...
#define RNPM_DMA_RESPLIT_SIZE (hw->dma_split_size >> 4)
	...
	dbg("%s: dma_split_size=%d page_size=%d rx_page_size=%d rx_offset=%d skb_shared_info=%d\n",
		__func__, hw->dma_split_size, PAGE_SIZE, rnpm_rx_pg_size(rx_ring), rnpm_rx_offset(rx_ring),
...
static int rnpm_vlan_rx_add_vid(struct net_device *netdev, __always_unused __be16 proto, u16 vid)
...
#endif /* HAVE_INT_NDO_VLAN_RX_ADD_VID */
{
	...
	if (adapter->flags & RNPM_FLAG_SRIOV_ENABLED) {
		...
		if (rd32(hw, RNPM_DMA_VERSION) >= 0x20201231) {
			for (port = 0; port < 4; port++) wr32(hw, RNPM_DMA_PORT_VEB_VID_TBL(port, vfnum), vid);
		} else {
...
#ifdef HAVE_INT_NDO_VLAN_RX_ADD_VID
#ifdef NETIF_F_HW_VLAN_CTAG_RX
static int rnpm_vlan_rx_kill_vid(struct net_device *netdev, __always_unused __be16 proto, u16 vid)
...
#endif
{
	...
	if (hw->mac.ops.set_vfta) {
		...
		// vid); 			} else { 				wr32(hw,
		// RNPM_DMA_PORT_VEB_VID_TBL(adapter->port, vfnum), vid);
		//			}
		//		}
		//	}
	}
...
static int rnpm_write_uc_addr_list(struct net_device *netdev)
{
	...
	if (!netdev_uc_empty(netdev)) {
		...
		hw_dbg(hw, "%s: rar_entries:%d, uc_count:%d offset %d\n", __func__, rar_entries,
			   adapter->uc_off, netdev_uc_count(netdev));
		...
		netdev_for_each_uc_addr(ha, netdev)
		{
			...
			/* that's ok */
			hw->mac.ops.set_rar(hw, rar_entries, ha->addr, VMDQ_P(0), RNPM_RAH_AV);

			/* if use mac filter we should also set Unicast to mac */
			if (hw->mac.mc_location == rnpm_mc_location_mac) {
				hw->mac.ops.set_rar_mac(hw, rar_entries - adapter->uc_off, ha->addr, VMDQ_P(0),
										adapter->port);
			}
			rar_entries--;
			count++;
	...
	hw_dbg(hw, "%s: Clearing RAR[%d - %d]\n", __func__, adapter->uc_off + 1, rar_entries);
	for (; rar_entries > adapter->uc_off; rar_entries--) {
		hw->mac.ops.clear_rar(hw, rar_entries);
		if (hw->mac.mc_location == rnpm_mc_location_mac) {
			hw->mac.ops.clear_rar_mac(hw, rar_entries - adapter->uc_off, adapter->port);
		}
	}
...
void rnpm_set_rx_mode(struct net_device *netdev)
{
	...
	if (netdev->flags & IFF_PROMISC) {
		...
		fctrl |=
			(RNPM_FCTRL_UNICASE_BYPASS | RNPM_FCTRL_MULTICASE_BYPASS | RNPM_FCTRL_BROADCASE_BYPASS);
	...
	if (hw->mac.mc_location == rnpm_mc_location_mac) {
		...
		value = rd32(hw, RNPM_MAC_PKT_FLT(port));
		if (!(adapter->flags & RNPM_FLAG_SWITCH_LOOPBACK_EN)) {	 // switch-loopback mode mac
...
static bool rnpm_wait_irq_miss_check_done(struct rnpm_adapter *adapter)
{
	...
	for (q_idx = 0; q_idx < adapter->num_q_vectors; q_idx++) {
		if (test_bit(RNPM_IRQ_MISS_HANDLE_DONE, &adapter->q_vector[q_idx]->flags))
			return false;
	}
...
static void rnpm_fdir_filter_restore(struct rnpm_adapter *adapter)
{
	...
	*/
	...
	/* setup ntuple */
	hlist_for_each_entry_safe(filter, node2, &adapter->fdir_filter_list, fdir_node)
	{
		rnpm_fdir_write_perfect_filter(adapter->fdir_mode, hw, &filter->filter, filter->hw_idx,
...
void rnpm_vlan_stags_flag(struct rnpm_adapter *adapter)
{
	...
	if (adapter->flags2 & RNPM_FLAG2_VLAN_STAGS_ENABLED) {
		...
		wr32(hw, RNPM_MAC_TX_VLAN_TAG(port),
			 RNPM_ERIVLT | RNPM_EDVLP | RNPM_ETV | (RNPM_EVLS_ALWAYS_STRIP << RNPM_EVLS_OFFSET) |
				 RNPM_VL_MODE_OFF);
		...
		wr32(hw, RNPM_MAC_TX_VLAN_TAG(port), RNPM_VTHM | RNPM_VL_MODE_ON | RNPM_ETV);
...
static void rnpm_configure(struct rnpm_adapter *adapter)
{
	...
	hw->mac.ops.set_rar(hw, adapter->uc_off, hw->mac.addr, VMDQ_P(0), RNPM_RAH_AV);
...
void control_mac_rx(struct rnpm_adapter *adapter, bool on)
{
	...
	if (on) {
		...
		// clean loop back
		do {
			wr32(hw, RNPM_MAC_RX_CFG(port), rd32(hw, RNPM_MAC_RX_CFG(port)) & (~0x400));
			usleep_range(100, 200);
			value = rd32(hw, RNPM_MAC_RX_CFG(port));
			count++;
			if (count > 1000) {
...
void rnpm_reinit_locked(struct rnpm_adapter *adapter)
{
	...
	while (test_and_set_bit(__RNPM_RESETTING, &adapter->state)) usleep_range(1000, 2000);
	rnpm_down(adapter);
	/*
...
void rnpm_reset(struct rnpm_adapter *adapter)
{
	...
	while (test_and_set_bit(__RNPM_IN_SFP_INIT, &adapter->state)) usleep_range(1000, 2000);

	/* clear all SFP and link config related flags while holding SFP_INIT */
	adapter->flags2 &= ~(RNPM_FLAG2_SEARCH_FOR_SFP | RNPM_FLAG2_SFP_NEEDS_RESET);
	...
	*/
	...
	if (module_enable_ptp) {
		if (adapter->flags2 & RNPM_FLAG2_PTP_ENABLED && (adapter->ptp_rx_en || adapter->ptp_tx_en))
	...
	while (i != rx_ring->next_to_alloc) {
		...
		if (rx_buffer->skb) {
			...
			if (rnpm_close_active_frag_list(skb))
				dma_unmap_single(rx_ring->dev, RNPM_CB(skb)->dma, rx_ring->rx_buf_len,
								 DMA_FROM_DEVICE);
			RNPM_CB(skb)->dma = 0;
#endif /* CONFIG_RNPM_DISABLE_PACKET_SPLIT */
			dev_kfree_skb(skb);
...
static void rnpm_clean_all_rx_rings(struct rnpm_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_rx_queues; i++) rnpm_clean_rx_ring(adapter->rx_ring[i]);
}
...
static void rnpm_clean_all_tx_rings(struct rnpm_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_tx_queues; i++) rnpm_clean_tx_ring(adapter->tx_ring[i]);
}
...
static void rnpm_fdir_filter_exit(struct rnpm_adapter *adapter)
{
	...
	{
		/* call earase to hw */
		rnpm_fdir_erase_perfect_filter(adapter->fdir_mode, hw, &filter->filter, filter->hw_idx);
...
void rnpm_down(struct rnpm_adapter *adapter)
{
	...
	rnpm_dbg("%s rnpm_down port=%d!!!\n", netdev->name, adapter->port);
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		...
		if (((adapter->flags & RNPM_FLAG_SRIOV_ENABLED) ||
			...
			head = rd32(hw, RNPM_DMA_REG_RX_DESC_BUF_HEAD(adapter->rx_ring[i]->rnpm_queue_idx));
			adapter->rx_ring[i]->ring_flags &= (~RNPM_RING_FLAG_CHANGE_RX_LEN);
			/* we should delay setup rx length to wait rx head to 0 */
			if (head >= adapter->rx_ring[i]->reset_count) {
				adapter->rx_ring[i]->ring_flags |= RNPM_RING_FLAG_DELAY_SETUP_RX_LEN;
	...
	adapter->flags2 &= ~(RNPM_FLAG2_FDIR_REQUIRES_REINIT | RNPM_FLAG2_RESET_REQUESTED);
	...
	if (!retry) {
		printk("error: %s wait ire miss check done timeout\n", netdev->name);
	}
	...
	for (i = 0; i < adapter->num_tx_queues; i++) {
		...
		if (timeout >= 100) {
			head = rd32(hw, RNPM_DMA_REG_TX_DESC_BUF_HEAD(tx_ring->rnpm_queue_idx));

			tail = rd32(hw, RNPM_DMA_REG_TX_DESC_BUF_TAIL(tx_ring->rnpm_queue_idx));
			printk("wait tx ready timeout, name=%s, i=%d queue_idx=%d head=%d "
				   "tail=%d\n",
		...
		if ((adapter->flags & RNPM_FLAG_SRIOV_ENABLED) ||
			...
			if (head != 0) {
				...
				if (head != (count - 1)) {
					/* 3 set len head + 1 */
					wr32(hw, RNPM_DMA_REG_TX_DESC_BUF_LEN(tx_ring->rnpm_queue_idx), head + 1);
...
static int rnpm_sw_init(struct rnpm_adapter *adapter)
{
	...
	adapter->ring_feature[RING_F_RSS].limit = min_t(int, rss, adapter->max_ring_pair_counts);
	...
	if (hw->feature_flags & RNPM_NET_FEATURE_RX_NTUPLE_FILTER) {
		...
		adapter->fdir_pballoc = adapter->layer2_count_max + adapter->tuple_5_count_max;
		// adapter->flags |= RNPM_FLAG_FDIR_PERFECT_CAPABLE;
	}
#ifdef CONFIG_RNPM_DCA
...
int rnpm_setup_tx_resources(struct rnpm_ring *tx_ring, struct rnpm_adapter *adapter)
{
	...
	DPRINTK(IFUP, INFO,
			...
			tx_ring->rnpm_queue_idx, tx_ring->q_vector->v_idx, tx_ring->count, tx_ring->desc,
			tx_ring->dma, numa_node);
	return 0;
err:
	rnpm_err("%s [SetupTxResources] #%d TxRing:%d,"
			 "vector:%d ItemCounts:%d\n",
			 tx_ring->netdev->name, tx_ring->queue_index, tx_ring->rnpm_queue_idx,
...
err_setup_tx:
	...
	while (i--) rnpm_free_tx_resources(adapter->tx_ring[i]);
	return err;
}
 ...
 * Returns 0 on success, negative on failure
 **/
int rnpm_setup_rx_resources(struct rnpm_ring *rx_ring, struct rnpm_adapter *adapter)
{
	...
	DPRINTK(IFUP, INFO,
			...
			rx_ring->rnpm_queue_idx, rx_ring->q_vector->v_idx, rx_ring->count, rx_ring->desc,
...
err:
	rnpm_err("%s [SetupRxResources] #%d RxRing:%d, vector:%d "
			 "ItemCounts:%d error!\n",
			 rx_ring->netdev->name, rx_ring->queue_index, rx_ring->rnpm_queue_idx,
...
static int rnpm_setup_all_rx_resources(struct rnpm_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		...
		if (unlikely(head >= adapter->rx_ring[i]->count)) {
			dbg("[%s] Ring %d head large than count", adapter->netdev->name,
...
err_setup_rx:
	/* rewind the index freeing the rings as we go */
	while (i--) rnpm_free_rx_resources(adapter->rx_ring[i]);
	return err;
}
...
static void rnpm_free_all_tx_resources(struct rnpm_adapter *adapter)
{
	...
	for (i = 0; i < (adapter->num_tx_queues); i++) rnpm_free_tx_resources(adapter->tx_ring[i]);
}
 ...
 * @maxrate: desired maximum transmit bitrate Mbps
 **/
static int rnpm_tx_maxrate(struct net_device *netdev, int queue_index, u32 maxrate)
{
	struct rnpm_adapter *adapter   = netdev_priv(netdev);
	struct rnpm_ring	*tx_ring   = adapter->tx_ring[queue_index];
	u64					 real_rate = 0;
...
void rnpm_update_stats(struct rnpm_adapter *adapter)
{
	...
	if (test_bit(__RNPM_DOWN, &adapter->state) || test_bit(__RNPM_RESETTING, &adapter->state))
		return;
	...
	switch (hw->mode) {
		case MODE_NIC_MODE_1PORT_40G:
		case MODE_NIC_MODE_1PORT:
			hw_stats->dma_to_eth = rd32(hw, RNPM_DMA_STATS_DMA_TO_DMA_CHANNEL_0) +
		...
		case MODE_NIC_MODE_2PORT:
			...
			for (i = port * 2; i < (port + 1) * 2; i++) {
				hw_stats->dma_to_eth += rd32(hw, RNPM_DMA_STATS_DMA_TO_DMA_CHANNEL(i));
			}
			break;
		case MODE_NIC_MODE_4PORT:
			hw_stats->dma_to_eth = rd32(hw, RNPM_DMA_STATS_DMA_TO_DMA_CHANNEL(port));
			break;
	}
	...
	rx_crc_error			 = rnpm_recalculate_err_pkts(rd32(hw, RNPM_RXTRANS_CRC_ERR_PKTS(port)),
	...
	net_stats->rx_errors +=
		...
		rnpm_recalculate_err_pkts(rd32(hw, RNPM_RXTRANS_CSUM_ERR_PKTS(port)),
								  &hw->err_pkts_init.csum[port], false) +
		rnpm_recalculate_err_pkts(hw_csum_rx_error, &hw->err_pkts_init.scsum[port], true) +
		rx_crc_error;
	net_stats->rx_dropped += rnpm_recalculate_err_pkts(rd32(hw, RNPM_RXTRANS_CUT_ERR_PKTS(port)),
													   &hw->err_pkts_init.cut[port], false) +
							 rnpm_recalculate_err_pkts(rd32(hw, RNPM_RXTRANS_DROP_PKTS(port)),
	...
	hw_stats->mac_rx_broadcast += ((u64)rd32(hw, RNPM_MAC_STATS_BROADCAST_HIGH(port)) << 32);

	// maybe no use
	hw_stats->mac_rx_multicast = rd32(hw, RNPM_MAC_STATS_MULTICAST_LOW(port));
	hw_stats->mac_rx_multicast += ((u64)rd32(hw, RNPM_MAC_STATS_MULTICAST_HIGH(port)) << 32);

	/* store to net_stats */
	net_stats->multicast	   = hw_stats->mac_rx_multicast;
	hw_stats->mac_tx_pause_cnt = rd32(hw, RNPM_MAC_STATS_TX_PAUSE_LOW(port));
	hw_stats->mac_tx_pause_cnt += ((u64)rd32(hw, RNPM_MAC_STATS_TX_PAUSE_HIGH(port)) << 32);

	hw_stats->mac_rx_pause_cnt = rd32(hw, RNPM_MAC_STATS_RX_PAUSE_LOW(port));
	hw_stats->mac_rx_pause_cnt += ((u64)rd32(hw, RNPM_MAC_STATS_RX_PAUSE_HIGH(port)) << 32);
}
...
static void rnpm_check_hang_subtask(struct rnpm_adapter *adapter)
{
	...
	if (netif_carrier_ok(adapter->netdev)) {
		for (i = 0; i < adapter->num_tx_queues; i++) set_check_for_tx_hang(adapter->tx_ring[i]);
	}
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		...
		if (rx_next_to_clean == rx_next_to_clean_old) {
			...
			if ((rx_ring->rx_stats.rx_equal_count > 2) && (rx_ring->rx_stats.rx_equal_count < 5)) {
		...
		if (rnpm_port_is_valid(pf_adapter, i)) {
			if (pf_adapter->adapter[i]) {
				...
				if (hw->phy_type == PHY_TYPE_SGMII) {
					/* get an */
					err = rnpm_mbx_phy_read(hw, 0, &hw->phy.vb_r[0]);
					if (err)
						goto error;
					hw->phy.an = (hw->phy.vb_r[0] & BIT(12)) ? AUTONEG_ENABLE : AUTONEG_DISABLE;
					err		   = rnpm_mbx_phy_read(hw, 4, &hw->phy.vb_r[4]);
					if (err)
						goto error;
					err = rnpm_mbx_phy_read(hw, 9, &hw->phy.vb_r[9]);
					if (err)
						goto error;
					err = rnpm_mbx_phy_read(hw, 17, &hw->phy.vb_r[17]);
					if (err)
						goto error;
					hw->phy.is_mdix = !!(hw->phy.vb_r[17] & 0x0040);
				}
			}
		}
	}
...
static int rnpm_watchdog_update_link(struct rnpm_adapter *adapter)
{
	...
	if (hw->mac.ops.check_link) {
		...
		link_up	   = true;
		rnpm_logd(LOG_LINK_EVENT, "WARN: %s:%s: check_link is null, force speed/link\n", __func__,
				  adapter->netdev->name);
	}

	if (link_up || time_after(jiffies, (adapter->link_check_timeout + RNPM_TRY_LINK_TIMEOUT))) {
		adapter->flags &= ~RNPM_FLAG_NEED_LINK_UPDATE;
	}
		   ...
		   (link_speed == RNPM_LINK_SPEED_10GB_FULL
				...
				: (link_speed == RNPM_LINK_SPEED_1GB_FULL
					   ...
					   : (link_speed == RNPM_LINK_SPEED_100_FULL
							  ? "100 Mbps"
							  : (link_speed == RNPM_LINK_SPEED_10_FULL ? "10 Mbps"
																	   : "unknown speed")))),
...
static void rnpm_fix_dma_tx_status(struct rnpm_pf_adapter *pf_adapter)
{
	...
	for (i = 0; i < 128; i++) wr32(pf_adapter, RNPM_DMA_TX_START(i), 1);
}
...
#define TSRN10_REG_DEBUG_VALUE (0x1a2b3c4d)
	...
	/* set all rx drop */
	for (i = 0; i < 4; i++) wr32(pf_adapter, RNPM_ETH_RX_PROGFULL_THRESH_PORT(i), DROP_ALL_THRESH);
...
__maybe_unused static void rnpm_update_feature_subtask(struct rnpm_adapter *adapter)
{
	struct rnpm_pf_adapter __maybe_unused *pf_adapter = adapter->pf_adapter;
	u32									   changed	  = 0;
...
{
	struct rnpm_pf_adapter *pf_adapter = container_of(work, struct rnpm_pf_adapter, service_task);
	...
	if (test_and_clear_bit(RNPM_PF_SERVICE_SKIP_HANDLE, &pf_adapter->flags)) {
		return;
	}
...
{
	struct rnpm_adapter *adapter = container_of(work, struct rnpm_adapter, service_task);
...
static int rnpm_tso(struct rnpm_ring *tx_ring, struct rnpm_tx_buffer *first, u8 *hdr_len)
{
	...
	if (skb_shinfo(skb)->gso_type & (SKB_GSO_GRE |
		...
		if (skb_shinfo(skb)->gso_type & (SKB_GSO_UDP_TUNNEL | SKB_GSO_UDP_TUNNEL_CSUM)) {
	...
	if (adapter->priv_flags & RNPM_PRIV_FLAG_TX_PADDING) {
		...
		if (gso_need_pad) {
			if ((gso_need_pad + *hdr_len) <= tx_ring->gso_padto_bytes) {
				gso_need_pad			= tx_ring->gso_padto_bytes - (gso_need_pad + *hdr_len);
...
static int rnpm_tx_map(struct rnpm_ring *tx_ring, struct rnpm_tx_buffer *first, const u8 hdr_len)
{
	struct sk_buff		  *skb = first->skb;
	struct rnpm_tx_buffer *tx_buffer;
	struct rnpm_tx_desc	  *tx_desc;
	...
	for (frag = &skb_shinfo(skb)->frags[0];; frag++) {
		...
		while (unlikely(size > RNPM_MAX_DATA_PER_TXD)) {
			...
			tx_desc->blen_mac_ip_len = cpu_to_le32(mac_ip_len ^ RNPM_MAX_DATA_PER_TXD);
			//==== desc==
	...
	tx_desc->vlan_cmd		 = cpu_to_le32(tx_flags | RNPM_TXD_CMD_EOP | RNPM_TXD_CMD_RS);
	tx_desc->blen_mac_ip_len = cpu_to_le32(mac_ip_len ^ size);
	...
	dma_unmap_single(tx_ring->dev, dma_unmap_addr(first, dma), dma_unmap_len(first, len),
					 DMA_TO_DEVICE);
...
__maybe_unused static void rnpm_atr(struct rnpm_ring *ring, struct rnpm_tx_buffer *first)
{
#if 0
	struct rnpm_q_vector *q_vector = ring->q_vector;
...
netdev_tx_t rnpm_xmit_frame_ring(struct sk_buff *skb, struct rnpm_adapter *adapter,
								 struct rnpm_ring *tx_ring)
{
	...
	*/
	...
	if (unlikely(skb_tx(skb)->hardware) && adapter->flags2 & RNPM_FLAG2_PTP_ENABLED &&
		adapter->ptp_tx_en) {
		if (!test_and_set_bit_lock(__RNPM_PTP_TX_IN_PROGRESS, &adapter->state)) {
...
static netdev_tx_t rnpm_xmit_frame(struct sk_buff *skb, struct net_device *netdev)
{
	struct rnpm_adapter *adapter = netdev_priv(netdev);
	struct rnpm_ring	*tx_ring;
	u8					 padto_bytes;
	 ...
	 */
	padto_bytes = skb_need_padto_bytes(skb, !!(adapter->priv_flags & RNPM_PRIV_FLAG_TX_PADDING));
...
static int rnpm_mdio_read(struct net_device *netdev, int prtad, int devad, u16 addr)
{
	int					 rc		 = -EIO;
	struct rnpm_adapter *adapter = netdev_priv(netdev);
	struct rnpm_hw		*hw		 = &adapter->hw;
...
__maybe_unused static int rnpm_mdio_write(struct net_device *netdev, int prtad, int devad, u16 addr,
										  u16 value)
{
	struct rnpm_adapter *adapter = netdev_priv(netdev);
	struct rnpm_hw		*hw		 = &adapter->hw;
...
static void rnpm_get_stats64(struct net_device *netdev, struct rtnl_link_stats64 *stats)
#else
static struct rtnl_link_stats64 *rnpm_get_stats64(struct net_device		   *netdev,
...
//  type, 			  void *type_data) #elif
//  defined(HAVE_NDO_SETUP_TC_CHAIN_INDEX) static int __rnpm_setup_tc(struct
//  net_device *netdev, u32 handle, 		u32 chain_index,
...
#else
static netdev_features_t rnpm_fix_features(struct net_device *netdev, netdev_features_t features)
#endif
{
	struct rnpm_adapter *adapter = netdev_priv(netdev);
	// struct rnpm_pf_adapter *pf_adapter = adapter->pf_adapter;
...
#else
static int rnpm_set_features(struct net_device *netdev, netdev_features_t features)
#endif
{
	struct rnpm_adapter *adapter = netdev_priv(netdev);
	// struct rnpm_pf_adapter *pf_adapter = adapter->pf_adapter;
	...
	if (changed & (NETIF_F_RXHASH)) {
		if (adapter->flags & RNPM_FLAG_MUTIPORT_ENABLED) {
			...
			u32 iov_en = (adapter->flags & RNPM_FLAG_SRIOV_ENABLED) ? RNPM_IOV_ENABLED : 0;
			/* close rxhash will lead all rx packets to ring 0 */
			if (features & (NETIF_F_RXHASH))
				wr32(hw, RNPM_ETH_RSS_CONTROL, RNPM_ETH_ENABLE_RSS_ONLY | iov_en);
			else
...
static int rnpm_ndo_bridge_getlink(struct sk_buff *skb, u32 pid, u32 seq, struct net_device *dev,
...
void rnpm_clear_udp_tunnel_port(struct rnpm_adapter *adapter)
{
	...
	// vxlanctrl = RNPM_READ_REG(hw, RNPM_VXLANCTRL) & ~mask;
	// RNPM_WRITE_REG(hw, RNPM_VXLANCTRL, vxlanctrl);
													...
													struct udp_tunnel_info *ti)
{
	...
	switch (ti->type) {
		case UDP_TUNNEL_TYPE_VXLAN:
			...
			if (adapter->vxlan_port) {
				netdev_info(dev, "VXLAN port %d set, not adding port %d\n",
		...
		case UDP_TUNNEL_TYPE_GENEVE:
			if (!(adapter->flags & RNPM_FLAG_GENEVE_OFFLOAD_CAPABLE))
				return;
	...
	// reg = RNPM_READ_REG(hw, RNPM_VXLANCTRL) | ntohs(port) << port_shift;
	// RNPM_WRITE_REG(hw, RNPM_VXLANCTRL, reg);
													...
													struct udp_tunnel_info *ti)
{
	...
	switch (ti->type) {
		...
		case UDP_TUNNEL_TYPE_GENEVE:
			...
			port_mask = RNPM_VXLANCTRL_GENEVE_UDPPORT_MASK;
			break;
		*/
		default:
			return;
 ...
 */
static void __maybe_unused rnpm_add_vxlan_port(struct net_device *dev, sa_family_t sa_family,
											   __be16 port)
{
	struct rnpm_adapter *adapter = netdev_priv(dev);
	struct rnpm_hw		*hw		 = &adapter->hw;
	...
	if (adapter->vxlan_port) {
		netdev_info(dev, "Hit Max num of VXLAN ports, not adding port %d\n", ntohs(port));
		return;
	}

	adapter->vxlan_port = port;
	wr32(hw, RNPM_ETH_VXLAN_PORT, adapter->vxlan_port);
	// RNPM_WRITE_REG(hw, RNPM_VXLANCTRL, ntohs(port));
}
 ...
 */
static void __maybe_unused rnpm_del_vxlan_port(struct net_device *dev, sa_family_t sa_family,
											   __be16 port)
{
	struct rnpm_adapter *adapter = netdev_priv(dev);
...
static netdev_features_t rnpm_features_check(struct sk_buff *skb, struct net_device *dev,
											 netdev_features_t features)
{
	...
	if (unlikely(mac_hdr_len > RNPM_MAX_MAC_HDR_LEN))
		return features & ~(NETIF_F_HW_CSUM | NETIF_F_SCTP_CRC | NETIF_F_HW_VLAN_CTAG_TX |
							NETIF_F_TSO | NETIF_F_TSO6);

	network_hdr_len = skb_checksum_start(skb) - skb_network_header(skb);
	if (unlikely(network_hdr_len > RNPM_MAX_NETWORK_HDR_LEN))
		return features & ~(NETIF_F_HW_CSUM | NETIF_F_SCTP_CRC | NETIF_F_TSO | NETIF_F_TSO6);
											 ...
											 netdev_features_t features)
{
	...
	if (unlikely(skb_inner_mac_header(skb) - skb_transport_header(skb) > RNPM_MAX_TUNNEL_HDR_LEN))
 ...
 *
 **/
int rnpm_wol_supported(struct rnpm_adapter *adapter, u16 device_id, u16 subdevice_id)
{
...
void rnpm_fix_queue_number(struct rnpm_hw *hw)
{
	...
	if ((rnpm_info_tbl[adapter->pf_adapter->board_type]->adapter_cnt == 2) &&
		(hw->mode == MODE_NIC_MODE_4PORT)) {
		if ((adapter->pf_adapter->board_type == board_n10) ||
			(adapter->pf_adapter->board_type == board_n400_4x1G)) {
			rnpm_info_tbl[adapter->pf_adapter->board_type]->total_queue_pair_cnts = 64;
		}
	}
...
static int rnpm_init_msix_pf_adapter(struct rnpm_pf_adapter *pf_adapter)
{
	...
	total_msix_counts =
		min_t(int, rnpm_info_tbl[pf_adapter->board_type]->total_queue_pair_cnts, total_msix_counts);
	...
	pf_adapter->msix_entries = kcalloc(vectors, sizeof(struct msix_entry), GFP_KERNEL);
	if (!pf_adapter->msix_entries) {
		rnpm_err("alloc msix_entries faild!\n");
		return -ENOMEM;
	}

	for (vector = 0; vector < vectors; vector++) pf_adapter->msix_entries[vector].entry = vector;

	err = pci_enable_msix_range(pf_adapter->pdev, pf_adapter->msix_entries, min_vectors, vectors);
...
{
	pf_adapter->hw.mbx.ops.configure(&pf_adapter->hw,
									 pf_adapter->msix_entries[pf_adapter->other_irq].entry, false);

#ifdef RNPM_DISABLE_IRQ
	return;
#endif
	free_irq(pf_adapter->msix_entries[pf_adapter->other_irq].vector, pf_adapter);
...
static int rnpm_request_mbx_irq(struct rnpm_pf_adapter *pf_adapter)
{
	...
	snprintf(pf_adapter->name, 20, "rnpm%d%d-other%d", rnpm_is_pf1(pf_adapter->pdev),
			 pf_adapter->bd_number, pf_adapter->other_irq);
	err = request_irq(pf_adapter->msix_entries[pf_adapter->other_irq].vector, rnpm_msix_other, 0,
					  pf_adapter->name, pf_adapter);
...
static int rnpm_add_pf_adapter(struct pci_dev *pdev, struct rnpm_pf_adapter **ppf_adapter,
...
#ifdef FIX_VF_BUG
	rnpm_wr_reg(hw_addr_bar0 + (0x7982fc & (pci_resource_len(pdev, RNPM_NIC_BAR0) - 1)), 0);
#else
	rnpm_wr_reg(hw_addr_bar0 + (0x7982fc & (pci_resource_len(pdev, RNPM_NIC_BAR0) - 1)), 1);
#endif
...
#define RNPM_NIC_BAR4 (4)
	pf_adapter->hw_addr4 = pf_adapter->hw_addr = pcim_iomap(pdev, RNPM_NIC_BAR4, 0);
...
static int rnpm_rm_pf_adapter(struct pci_dev *pdev, struct rnpm_pf_adapter **ppf_adapter)
{
	...
	rnpm_wr_reg(pf_adapter->hw_bar0 + (0x7982fc & (pci_resource_len(pdev, RNPM_NIC_BAR0) - 1)), 1);
...
static int rnpm_add_adpater(struct pci_dev *pdev, const struct rnpm_info *ii,
							struct rnpm_adapter **padapter, struct rnpm_pf_adapter *pf_adapter,
							int port, int msix_offset, int port_name)
{
	int					 i, err = 0;
	...
	indices = queues;
	pr_info("====  add adapter queues:%d table %d ===", queues, pf_adapter->max_msix_counts[port]);
	...
	snprintf(adapter->name, sizeof(netdev->name), "%s%ds%df%d", rnpm_port_name, pdev->bus->number,
			 rnpm_is_pf1(pdev), adapter->port);
#else
	snprintf(adapter->name, sizeof(netdev->name), "%s%ds%df%d", rnpm_port_name, pdev->bus->number,
			 rnpm_is_pf1(pdev), port_name);
#endif
	if (padapter) {
		*padapter				= adapter;
	...
	hw->single_lane_link_evt_ctrl_ablity = pf_adapter->hw.single_lane_link_evt_ctrl_ablity;
	...
	memcpy(&hw->mac.ops, &pf_adapter->hw.mac.ops, sizeof(pf_adapter->hw.mac.ops));
	...
	pr_info("%s %s: dma version:0x%x, nic version:0x%x, pfvfnum:0x%x lane%d %p\n", adapter->name,
			pci_name(pdev), hw->dma_version, rd32(hw, RNPM_TOP_NIC_VERSION), hw->pfvfnum,
			hw->nr_lane, hw);
	...
	*/
	...
	netdev->extended->max_mtu = RNPM_MAX_JUMBO_FRAME_SIZE - (ETH_HLEN + 2 * ETH_FCS_LEN);
#else
	...
	adapter->pf_adapter->force_10g_1g_speed_ablity =
		rnpm_is_pf1(pdev) ? !!force_speed_ablity_pf1 : !!force_speed_ablity_pf0;
	if (hw->max_speed_1g) {
	...
	if (fix_eth_name) {
		if (!(adapter->flags & RNPM_FLAG_MUTIPORT_ENABLED)) {
			snprintf(adapter->name, sizeof(netdev->name), "rnp%d%d", rnpm_is_pf1(pdev),
					 adapter->bd_number);
		} else {
			snprintf(adapter->name, sizeof(netdev->name), "rnpm%d%d%d", rnpm_is_pf1(pdev),
	...
	if (adapter->flags & RNPM_FLAG_SRIOV_ENABLED) {
		...
		for (i = 0; i < adapter->num_vfs; i++) rnpm_vf_configuration(pdev, (i | 0x10000000));
	}
...
static int rnpm_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if ((rnpm_is_pf1(pf_adapter->pdev) == 0) && rnpm_can_rpu_start(pf_adapter)) {
	...
	do {
		...
		// do register
		err = rnpm_add_adpater(pdev, ii, &pf_adapter->adapter[lane_num], pf_adapter, lane_num,
...
static void rnpm_remove(struct pci_dev *pdev)
{
	...
	while (test_and_set_bit(__RNPM_RESETTING, &pf_adapter->state)) usleep_range(1000, 2000);
	...
	*/
	...
	//			IORESOURCE_MEM));
	dma_free_coherent(&pdev->dev, pf_adapter->hw.mbx.reply_dma_size, pf_adapter->hw.mbx.reply_dma,
 ...
 */
static pci_ers_result_t rnpm_io_error_detected(struct pci_dev *pdev, pci_channel_state_t state)
{
...
#ifdef FIX_VF_BUG
	// rnpm_wr_reg(pf_adapter->hw_bar0 + (0x7982fc &
	// (pci_resource_len(pdev, 0) - 1)), 0);
#endif
...
__maybe_unused static int __rnpm_shutdown(struct pci_dev *pdev, bool *enable_wake)
{
	struct rnpm_pf_adapter *pf_adapter = pci_get_drvdata(pdev);
	struct rnpm_adapter	   *adapter;
	int						i;
...
static struct pci_driver rnpm_driver = {.name	  = rnpm_driver_name,
