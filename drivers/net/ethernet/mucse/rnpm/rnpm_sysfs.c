...
static ssize_t store_pf_reset(struct device *dev, struct device_attribute *attr,
...
static DEVICE_ATTR(active_vid, S_IRUGO | S_IWUSR, show_active_vid,
		   store_active_vid);
...
static ssize_t show_physical_ring_num(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	struct net_device *netdev = to_net_device(dev);
	struct rnpm_adapter *adapter = netdev_priv(netdev);
...
static DEVICE_ATTR(ring_num, S_IRUGO | S_IRUSR, show_physical_ring_num, NULL);
...
static ssize_t show_nr_lane(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	int ret = 0;
	struct net_device *netdev = to_net_device(dev);
...
static ssize_t show_debug_aptstat(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	int ret = 0;
	struct net_device *netdev = to_net_device(dev);
...
static DEVICE_ATTR(debug_aptstat, S_IRUGO | S_IRUSR, show_debug_aptstat, NULL);
...
static ssize_t show_bar4_reg(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	int ret = 0;
...
static ssize_t store_bar4_reg(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct net_device *netdev = to_net_device(dev);
	struct rnpm_adapter *adapter = netdev_priv(netdev);
...
static ssize_t show_phy_statistics(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int ret = 0;
	struct net_device *netdev = to_net_device(dev);
...
static DEVICE_ATTR(phy_statistics, S_IRUGO | S_IRUSR, show_phy_statistics,
		   NULL);
...
static ssize_t pma_loopback(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	struct net_device *netdev = to_net_device(dev);
	struct rnpm_adapter *adapter = netdev_priv(netdev);
				  ...
				  int sport_lane, int dst_switch_port)
{
	...
	printk("%s: %s %d -> %d en:%d\n", __func__,
	       netdev_name(adapter->netdev), sport_lane, dst_switch_port, en);
			       ...
			       unsigned int offset, int type)
{
	...
	rnpm_attr->dev_attr.attr.mode = S_IRUGO;
