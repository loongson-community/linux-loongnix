...
static int
rnpvf_vlan_rx_add_vid(struct net_device *netdev, __be16 proto, u16 vid)
{
	struct rnpvf_adapter *adapter = netdev_priv(netdev);
	struct rnpvf_hw *hw = &adapter->hw;
	int err = 0;
...
static int
rnpvf_vlan_rx_kill_vid(struct net_device *netdev, __be16 proto, u16 vid)
{
	struct rnpvf_adapter *adapter = netdev_priv(netdev);
	struct rnpvf_hw *hw = &adapter->hw;
	int err = -EOPNOTSUPP;
...
static void remove_mbx_irq(struct rnpvf_adapter *adapter)
{
	u32 msgbuf[2];
	struct rnpvf_hw *hw = &adapter->hw;
...
// 
static int register_mbx_irq(struct rnpvf_adapter *adapter)
{
        struct rnpvf_hw *hw = &adapter->hw;
        struct net_device *netdev = adapter->netdev;
        int err = 0;
