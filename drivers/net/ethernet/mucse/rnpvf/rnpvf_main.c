...
static struct pci_device_id rnpvf_pci_tbl[] = {
	{ PCI_DEVICE(0x8848, 0x1080), .driver_data = N10_BOARD }, // N10
	{ PCI_DEVICE(0x8848, 0x1081),
	  .driver_data = N10_BOARD }, // N400-2port-VF
	{ PCI_DEVICE(0x8848, 0x1083),
	  .driver_data = N10_BOARD }, // N400-1port-VF
	{ PCI_DEVICE(0x8848, 0x1C80), .driver_data = N10_BOARD }, // N10C
	{ PCI_DEVICE(0x8848, 0x1C81),
	  .driver_data = N10_BOARD }, // N400C-2port-VF
	{ PCI_DEVICE(0x8848, 0x1C83),
	  .driver_data = N10_BOARD }, // N400-1port-VF
	/* required last entry */
	{
		0,
...
#define RNPVF_RSS_TYPE_MASK 0xc0
	...
	RHEL_RELEASE_VERSION(7,1) skb->rxhash = le32_to_cpu(rx_desc->wb.rss_hash);
	#else
	...
	*/
	...
	while (i != rx_ring->next_to_alloc) {
		...
		if (rx_buffer->skb) {
			...
			// defined(HAVE_SWIOTLB_SKIP_CPU_SYNC)
			//                                                      &attrs);
			// #else
			//                                                      RNP_RX_DMA_ATTR);
			// #endif
				  ...
				  struct sk_buff *skb)
{
	...
	//			if ((!(netdev->flags & IFF_PROMISC)) &&
	//				(!(netdev->features & NETIF_F_RXALL))) {
	//                                /* sctp less than 60 hw report err by
	//                                mistake */ if
	//                                (unlikely(rnpvf_test_staterr(
	//                                            rx_desc,
	//                                            RNP_RXD_STAT_SCTP_MASK))) {
	//                                        if (skb->len > 60) {
	//                                                dev_kfree_skb_any(skb);
...
static irqreturn_t rnpvf_msix_other(int irq, void *data)
{
	...
	dbg("\n\n !!! %s irq-coming !!!\n", __func__);
	// dev_info(&adapter->pdev->dev, "irq-coming\n");
	...
	// dev_info(&adapter->pdev->dev, "no polling irq-coming\n");
			     ...
			     struct rnpvf_ring *ring)
{
	...
	//ring_wr32(
	//		ring, RNP_DMA_REG_TX_FLOW_CTRL_TH,
...
rnpvf_configure_srrctl(struct rnpvf_adapter *adapter, int index)
{
#if 0
	...
	if (vid) {
		...
		if (hw->dma_version >= 0x20201231) {
			...
			for (port = 0; port < 4; port++) {
				wr32(hw, RNP_DMA_PORT_VEB_VID_TBL(port, VFNUM(mbx, hw->vfnum)),
		vid);
			}
		} else {
			wr32(
					hw, RNP_DMA_PORT_VEB_VID_TBL(adapter->port, VFNUM(mbx,
		hw->vfnum)), vid);
		}
		*/
	}
...
__maybe_unused static void
rnpvf_negotiate_api(struct rnpvf_adapter *adapter)
{
}
...
__maybe_unused static void
rnpvf_fdir_filter_exit(struct rnpvf_adapter *adapter)
{
}
...
#endif
{
	...
	// record the old rx chksum status
...
// #endif
//  rnpvf_get_stats64(struct net_device *netdev, struct rtnl_link_stats64
//  *stats)
