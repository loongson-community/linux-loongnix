...
static const struct snd_kcontrol_new es8326_snd_controls[] = {
	SOC_SINGLE_TLV("DAC Playback Volume", ES8326_DAC_VOL,
		       0, 0xbf, 0, dac_vol_tlv),
	SOC_ENUM("Playback Polarity", dacpol),
	SOC_SINGLE_TLV("DAC Ramp Rate", ES8326_DAC_RAMPRATE,
		       0, 0x0f, 0, softramp_rate),
	SOC_SINGLE_TLV("DRC Recovery Level", ES8326_DRC_RECOVERY,
		       0, 4, 0, drc_recovery_tlv),
	SOC_ENUM("DRC Winsize", drc_winsize),
	SOC_SINGLE_TLV("DRC Target Level", ES8326_DRC_WINSIZE,
		       0, 0x0f, 0, drc_target_tlv),

	SOC_DOUBLE_R_TLV("ADC Capture Volume", ES8326_ADC1_VOL,
			 ES8326_ADC2_VOL, 0, 0xff, 0, adc_vol_tlv),
	SOC_DOUBLE_TLV("ADC PGA Volume", ES8326_ADC_SCALE,
		       4, 0, 5, 0, adc_pga_tlv),
	SOC_SINGLE_TLV("ADC PGA Gain Volume", ES8326_PGAGAIN,
		       0, 10, 0, adc_analog_pga_tlv),
	SOC_SINGLE_TLV("ADC Ramp Rate", ES8326_ADC_RAMPRATE,
		       0, 0x0f, 0, softramp_rate),
...
static int es8326_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	...
	if (mute) {
		...
		if (!es8326->calibrated) {
			...
			msleep(30);
			regmap_write(es8326->regmap, ES8326_HP_CAL, 0x00);
			regmap_read(es8326->regmap, ES8326_HPL_OFFSET_INI,
				    &offset_l);
			regmap_read(es8326->regmap, ES8326_HPR_OFFSET_INI,
				    &offset_r);
			regmap_write(es8326->regmap, ES8326_HP_OFFSET_CAL,
				     0x8c);
			regmap_write(es8326->regmap, ES8326_HPL_OFFSET_INI,
				     offset_l);
			regmap_write(es8326->regmap, ES8326_HPR_OFFSET_INI,
				     offset_r);
...
static int es8326_set_bias_level(struct snd_soc_component *comp,
				 enum snd_soc_bias_level level)
{
	struct es8326_priv *es8326 = snd_soc_component_get_drvdata(comp);
	int ret;
	...
	case SND_SOC_BIAS_ON:
		...
		regmap_update_bits(es8326->regmap,  ES8326_CLK_CTL,
				   0x20, 0x20);
...
static void es8326_enable_micbias(struct snd_soc_component *comp)
{
...
static void es8326_disable_micbias(struct snd_soc_component *comp)
{
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(comp);
...
static void es8326_jack_detect_handler(struct work_struct *work)
{
	...
	if (es8326->jack_remove_retry == 1) {
		...
		/*
		 * Inverted HPJACK_POL bit to trigger one IRQ to
		 * double check HP Removal event
		 */
	...
	if (!(iface & ES8326_HPINSERT_FLAG)) {
		...
		if (es8326->hp == 0) {
			/*
			 * Set auto-check mode, then restart jack_detect_work
			 * after 100ms. Don't report jack status.
			 */
			regmap_update_bits(es8326->regmap, ES8326_HPDET_TYPE,
					   0x03, 0x01);
...
static int es8326_calibrate(struct es8326_priv *es8326)
{
	unsigned int reg;
	unsigned int offset_l, offset_r;
...
static void es8326_init(struct es8326_priv *es8326)
{
	unsigned int reg;
...
static int es8326_suspend(struct snd_soc_component *comp)
{
	struct es8326_priv *es8326 = snd_soc_component_get_drvdata(comp);
...
static int es8326_resume(struct snd_soc_component *comp)
{
	struct es8326_priv *es8326 = snd_soc_component_get_drvdata(comp);
	int ret;

	if (es8326->mclk)
		clk_prepare_enable(es8326->mclk);
	...
	if (ret) {
		dev_err(comp->dev, "unable to sync regcache\n");
		return ret;
	}
...
static int es8326_probe(struct snd_soc_component *component)
{
	...
	ret = device_property_read_u8(component->dev, "everest,mic1-src",
				      &es8326->mic1_src);
	if (ret != 0)
		es8326->mic1_src = ES8326_ADC_AMIC;

	ret = device_property_read_u8(component->dev, "everest,mic2-src",
				      &es8326->mic2_src);
	if (ret != 0)
	...
	ret = device_property_read_u8(component->dev, "everest,jack-pol",
				      &es8326->jack_pol);
	if (ret != 0)
		es8326->jack_pol = ES8326_HP_DET_BUTTON_POL |
				   ES8326_HP_TYPE_OMTP;
			    ...
			    const struct i2c_device_id *id)
{
	...
	es8326 = devm_kzalloc(&i2c->dev, sizeof(*es8326), GFP_KERNEL);
	if (!es8326)
		return -ENOMEM;
	...
	es8326->mclk = devm_clk_get(&i2c->dev, "mclk");
