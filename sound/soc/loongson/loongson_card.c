...
static char codec_name[ACPI_ID_LEN + 8];
...
static int loongson_card_parse_acpi(struct loongson_card_data *data)
{
	...
	for (i = 0; i < card->num_links; i++)
		loongson_dai_links[i].platform_name = dev_name(phy_dev);
	...
	for (i = 0; i < card->num_links; i++)
		loongson_dai_links[i].codec_name = codec_name;

	device_property_read_string(card->dev, "codec-dai-name",
				    &codec_dai_name);
	for (i = 0; i < card->num_links; i++)
		loongson_dai_links[i].codec_dai_name = codec_dai_name;
...
static int loongson_card_parse_of(struct loongson_card_data *data)
{
	...
	if (ret < 0) {
		dev_err(dev, "cpu node missing #sound-dai-cells\n");
		goto err;
	}
	for (i = 0; i < card->num_links; i++) {
		loongson_dai_links[i].cpu_of_node = args.np;
		loongson_dai_links[i].platform_of_node = args.np;

		ret = snd_soc_get_dai_name(&args, &cpu_dai_name);
		if (ret < 0) {
			of_node_put(args.np);
			dev_warn(dev, "Unable to get cpu_dai_name\n");
			goto err;
		}
		loongson_dai_links[i].cpu_dai_name = cpu_dai_name;
	}
	...
	for (i = 0; i < card->num_links; i++) {
		loongson_dai_links[i].codec_of_node = args.np;

		ret = snd_soc_get_dai_name(&args, &codec_dai_name);
		if (ret < 0) {
			dev_warn(dev, "Unable to get codec_dai_name\n");
			goto err;
		}
		loongson_dai_links[i].codec_dai_name = codec_dai_name;
	}
...
{
	struct loongson_card_data *ls_priv = NULL;
	...
	ls_priv->snd_card.dev = &pdev->dev;
	ls_priv->snd_card.owner = THIS_MODULE;
	ls_priv->snd_card.dai_link = loongson_dai_links;
	ls_priv->snd_card.num_links = ARRAY_SIZE(loongson_dai_links);
	...
	platform_set_drvdata(pdev, ls_priv);
...
MODULE_SOFTDEP("pre: snd-soc-loongson-i2s-plat snd-soc-loongson-i2s-pci");
